...
import { Button, Card, Row, Col, Icon, Divider } from 'antd'
import EditImpactReportForm from 'src/containers/Reports/EditImpactReportForm'
...
class ProjectImpact extends PureComponent {
  ...
  toggleEditImpact = report => this.setState({ report, editImpact: !this.state.editImpact })
    ...
    return (
      <Fragment key={idx}>
        <Row type="flex" justify="space-between" gutter={24}>
          <Col span={13}>
            <div className={styles.reportName}>{indicators[report.indicator]} <Button shape="circle" icon="edit" size="small" type="primary" ghost onClick={() => this.toggleEditImpact(report)} /></div>
  ...
  render() {
    const { reports = [], projectId } = this.props
    const { report, editImpact } = this.state
    ...
    return (
      <Card>
        ...
        <EditImpactReportForm
          visible={editImpact}
          projectId={projectId}
          report={report}
          onCancel={() => this.toggleEditImpact()}
        />
      </Card>
    )
  }
