  ...
  renderSDGicons(sdg) {
    ...
    return (
      <div className={styles.goals}>
        ...
        <div className={styles.goal}>{text}</div>
      </div>
    )
  }

  render() {
    const { loading, framework, actions, className } = this.props
    if (loading)
      return (
        <Fragment>
          <div className={styles.frameworkTitleContainer}>
            <div className={styles.frameworkTitle}>Framework</div>
            {actions}
          </div>
    ...
    return (
      <Fragment>
        ...
        <Card className={`${styles.frameworkContainer} ${className}`}>
          <Row type="flex" justify="space-between" gutter={24}>
            ...
            <Col span={9} offset={1}>
              ...
              {framework.categories.map(category => (
                <div key={category.tuid} className={styles.category}>
                  {category.text}
                </div>
              ))}
              <Divider />
              <Row type="flex" justify="space-between" gutter={24}>
                <Col span={24}>
                  <div className={styles.categoriesTitle}>Supporting Documentation</div>
                </Col>
                {framework.files.map(file => (
                  <Col span={framework.files.length > 1 ? 12 : 24} key={file.tuid} className={styles.file}>
                    <a
          ...
          <Row>
            <Col>
              ...
              {framework.goals.map(goal => (
                <div key={goal.tuid} className={styles.goal}>
